...
def _load_tool_class(module_name: str, class_name: str) -> type:
    ...
    module_path = root / "agent" / "tools" / f"{module_name}.py"
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Run the Crestline internal assistant agent.")
    parser.add_argument("query", nargs="+", help="Question to ask the agent.")
    parser.add_argument(
        "--category",
        choices=["hr", "marketing", "product", "sql"],
        help="Force a specific tool category.",
    )
    args = parser.parse_args()

    agent = Agent()
    prompt = " ".join(args.query)
    state = agent.run(prompt, force_category=args.category)
